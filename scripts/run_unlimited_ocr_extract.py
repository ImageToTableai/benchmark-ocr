#!/usr/bin/env python3
"""Run Unlimited-OCR through an OpenAI-compatible vLLM server for field extraction.

Reuses the field-schema scaffolding introduced by run_surya2_extract.py so the
emitted ``fields`` block aligns with the dataset gt_fields shape that the
evaluator scores. Unlike Surya 2, Unlimited-OCR is a prompt-following VLM, so
the same chat/completions call can be asked to return structured JSON.
"""
...
SPECIAL_TOKEN_RE = re.compile(r"<\|(?:det|/det|ref|/ref)\|>", re.DOTALL)
...
def extract_json_object(text: str) -> Any:
    stripped = clean_special_tokens(text)
    if not stripped:
...
def main() -> int:
    ...
    parser.add_argument("--max-tokens", type=int, default=int(os.environ.get("UNLIMITED_OCR_EXTRACT_MAX_TOKENS", "1024")))
    ...
    raw_dir = out_path.parent / "raw" / "unlimited_ocr_extract"
